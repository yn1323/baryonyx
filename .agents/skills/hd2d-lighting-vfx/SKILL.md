...
Topのような`ScreenSpaceOverlay`背景で木漏れ日を作る場合は、`Hd2dLightShaft.prefab`を独立した光芒レイヤーとして使う。先端へ広がらない柔らかな長方形の光芒を、画面外の右上から画面外の左下へ通し、複数本を低い透明度で重ねて明るさを揺らす。`ShaftCount`、`ShaftColor`、`LengthRange`、`WidthRange`、`WidthScaleRange`、`RotationRange`、`SourceAnchor`、`FlickerAmount`、`MotionAmplitude`はPrefabのInspectorから調整し、画面固有の配置はシーンのPrefabインスタンスで上書きする。`SourceAnchor`は1を超える値を指定して画面外から開始できる。

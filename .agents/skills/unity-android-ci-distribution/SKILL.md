...
1. ルートと`client/`の`AGENTS.md`、`ProjectSettings/ProjectVersion.txt`、`Packages/manifest.json`、`client/Assets/Baryonyx/Editor/CI/AndroidBuild.cs`を読む。
...
- 標準の`shortcuts/build-apk.bat`または`shortcuts/build-apk-to-drive.bat`を使い、別のビルド経路を選ぶ場合も同じ`AndroidBuild.Build`を呼び出す。
- `client/Builds/Android/baryonyx.apk`、`client/Logs/build-apk.log`、`client/Logs/build-apk-to-drive.log`の実在と成功マーカーを確認する。
- ビルド失敗時は、古いAPKを新しい成果物として配布しない。既存の配置先を更新しない。
- APK生成後は、実行方法にかかわらず、指定されたDrive配置先へ上書きコピーする。配置先へアクセスできない、コピーに失敗する、対象フォルダーがない場合は作業を未完了として報告する。

現在のリポジトリで指定された個人Drive配置先は次のとおりである。

```text
G:\マイドライブ\71_プロジェクト\baryonyx\baryonyx.apk
```
...
- Android SDK、NDK、Gradle、JDKの組み合わせはUnity 6.6とプロジェクトのCI設定に合わせ、機械的に最新版へ変更しない。

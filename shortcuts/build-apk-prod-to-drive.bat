...
chcp 65001 >nul
rem Build an APK for the prod game server, ignoring any server settings in the caller environment.
set "BARYONYX_ENVIRONMENT=prod"
...
try {
    ...
    $destinationApk = Join-Path $destinationDirectory 'baryonyx-prod.apk'
    ...
    $buildLog = Join-Path $logDirectory 'build-apk-prod-to-drive.log'
    ...
    Write-Host "Unity $editorVersion でprod環境向けのAPKをビルドします。完了までこのウィンドウを開いたままにしてください。"
    ...
    $serverSelected = Select-String -LiteralPath $buildLog -SimpleMatch 'BARYONYX_ANDROID_SERVER: prod ' -Quiet
    if (-not $serverSelected) {
        throw "prod環境の接続先でビルドされたことを確認できませんでした。ビルドログを確認してください: $buildLog"
    }

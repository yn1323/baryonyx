...
import { hashToken } from "../../src/features/health/auth.js";
import {
...
describe("セッションの失効", () => {
  ...
  it("同じGoogleユーザーの再ログインでユーザーを維持し、各セッションを個別に失効できる", async () => {
    const { login, request } = scenario;
    const first = await login("repeat-subject");
    const second = await login("repeat-subject");
    expect(second.userId).toBe(first.userId);
    expect(second.token).not.toBe(first.token);
    expect(
      (await request("/auth/logout", "POST", undefined, first.token)).status,
    ).toBe(204);
    expect(
      (await request("/auth/logout", "POST", undefined, second.token)).status,
    ).toBe(204);
  });
